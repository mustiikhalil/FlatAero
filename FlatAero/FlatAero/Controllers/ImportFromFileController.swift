...
extension ImportFromFileController: BuildImportFileView, ImportFiles {
  ...
  func importFile(_ sender: Any) {
    ...
    Logging.logger.info("Importing \(type) file")
    switch type {
    case .fbsFile:
...
extension ImportFromFileController {
  ...
  func setupView() {
    ...
    importBinaryTable.anchorInSuperViewDisregarding(
      edges: .bottom,
      padding: padding)
    importTable.anchorInSuperViewDisregarding(
      edges: .top,
      .bottom,
      padding: padding)
    importTable.topAnchor.constraint(
      equalTo: pathBinaryTextField.bottomAnchor,
      constant: 10).isActive = true

    view.addSubview(fbsTextViewController.view)
    fbsTextViewController.view.topAnchor.constraint(
      equalTo: importFBSFileButton.bottomAnchor,
      constant: 10).isActive = true
    fbsTextViewController.view.anchorInSuperViewDisregarding(
      edges: .top,
      padding: padding)
  ...
  func setupBinaryImportView() {
    ...
    importerViewStack.topAnchor.constraint(
      equalTo: importBinaryTable.bottomAnchor,
      constant: 10).isActive = true
